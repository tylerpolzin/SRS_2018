...
//          START 'MY PRODUCTS' JS                                                  |

$(document).on("turbolinks:load", function() {
  var table = $('#employeeProductsDataTable').DataTable({
                // "scrollX": true,
                // "scrollY": true,
                "dom": '<"my-products-toolbar"><"col-md-12 glider-table"t><"col-md-12"ip>',
                "aLengthMenu": [[5, 10, 20, -1], [5, 10, 20, "All"]],
                "pageLength": 10,
                "bJQueryUI": true,
                "columnDefs": [
  ...
  $("div.my-products-toolbar").html(''+
    '<ul class="nav nav-tabs">'+
    ' <li><div class="dataTables_filter"><input class="form-control" id="myProductTableSearch" placeholder="Search Table..."></div>'+
    ' <li>'+
    '    <div class="dataTables_length">'+
    '     <select '+
    '       class="form-control"'+
    '       title="Number of records to show"'+
    '       id="myProductTableLength">'+
    '       <option value="5">5</option>'+
    ...
    '</ul>'+
    '');
  $('#myProductTableSearch').keyup(function(){
    table.search($(this).val()).draw() ;
  });
  $('#myProductTableLength').on("change", function(){
    table.page.len($(this).find("option:selected").attr("value")).draw() ;
  });

  function format (photo, upc) {
    return ''+
   '<div class="glider">'+
   '  <table class="my-product-listing-expando">'+
   '    <tr>'+
   '      <td>Product Photo</td>'+
   '      <td>UPC</td>'+
   '    </tr>'+
  ...
  $('#myProductsDataTable').on('click', 'td.details-control', function() {
    var tr = $(this).closest('tr');
    var td = $(tr).find('td:first-child');
    var row = table.row(tr);
    if (row.child.isShown()) {
      ...
      row.child(format(tr.data('child-photo'), tr.data('child-upc')), 'no-padding').show();
      tr.addClass('shown');
      td.tooltip('disable');
      $('div.glider', row.child()).slideDown();
    }
...
$(document).on("turbolinks:load", function() {
  var table = $('#drsharpProductsDataTable').DataTable({
                // "scrollX": true,
                // "scrollY": true,
                "dom": '<"my-products-toolbar"><"col-md-12 glider-table"t><"col-md-12"ip>',
                "aLengthMenu": [[5, 10, 20, -1], [5, 10, 20, "All"]],
